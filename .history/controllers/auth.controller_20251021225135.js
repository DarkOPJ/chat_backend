...
import User from "../models/user.js";
import { generate_and_send_jwt } from "../utils/jwt.js";

/**
 * Production-grade secure login handler
 * Protects against timing attacks and enumeration
 */
export const login = async (req, res) => {
  ...
  try {
    const user = await User.findOne({ email: processed_email }).select(
      "+password"
    );
    ...
    const token = generate_and_send_jwt(user._id, res);
