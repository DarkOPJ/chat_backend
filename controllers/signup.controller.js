...
const signup = async (req, res) => {
  ...
  const processed_full_name = typeof email === "string" ? full_name.trim() : "";
  ...
  try {
    ...
    const existing_user = await User.findOne({ processed_email });
    if (existing_user) {
      return res.status(400).json({
        message:
    ...
    // you can equally use one line instead of 2 with const hashed_password = await bcrypt.hash(processed_password, 10)
